...
from nets.SegNets import HybridEfficientSeg,TraditionalUnet
...
if __name__ == "__main__":
    ...
    # ---------------------------------#
    wandb_config = {
        "project": "unet-segmentation_dataA",  # 项目名称
        "entity": None,  # 你的wandb用户名
        "name": "test-experiment",  # 实验名称
    }
    ...
    model = TraditionalUnet(num_classes=num_classes).train()
    if not pretrained:
        weights_init(model)
    if model_path != '':
        if local_rank == 0:
